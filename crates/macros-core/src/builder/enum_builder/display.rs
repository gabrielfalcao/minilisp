...
impl EnumBuilder {
    pub fn build_enum_display_impl(&self) -> Result<TokenStream> {
        let name = self.name();
        let str_variants = try_result!(self.impl_display_variants());
        let stream = quote! {
    ...
    pub fn impl_display_variants(&self) -> Result<TokenStream> {
